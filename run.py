...
vertexai.init(project="global-sign-414606", location="us-central1")
model = GenerativeModel("gemini-pro")

def generate_from_gemini(text):
...
text_for_schema=f'create a dataframe using the given {schema} and then using sqlalchemy to generate the schema and then generate 30 records for it and put it in the database also create a my_table_schema_objs which has the format SQLTableSchema(table_name="your_table_name", context_str="Your table summary"),) for all the tables in the schema and database_url="sqlite:///:memory:" and dont generate the language of the code and dont generated header'
generated_list=generate_from_gemini(text_for_schema)

with open('test.py','w') as f:
    for con in generated_list:
        f.write(con)
...
additional=additional.replace("python",'')

with open('train.py','r') as f:
    content=f.read()

content_write=f"{additional}\n\n{content}"
...
# content=generate_from_gemini(sql_query+" give a javascript function to execute this query")
